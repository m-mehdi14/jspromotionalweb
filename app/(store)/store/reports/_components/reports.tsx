...
export const StoreReports: React.FC<StoreReportsProps> = ({
  metrics,
  specialEventsData,
  flyersData,
  couponData,
  ...
  const handleApplyDateRange = () => {
    router.push(`/store/reports?startDate=${startDate}&endDate=${endDate}`);
  };

  const handleClearDateRange = () => {
    setStartDate("");
    setEndDate("");
    router.push("/store/reports");
  };

  const handleGenerateReport = () => {
    const doc = new jsPDF();
    doc.text("Store Report", 14, 20);
    doc.text(`Generated on: ${new Date().toLocaleString()}`, 14, 30);
    if (startDate && endDate)
      doc.text(`Date Range: ${startDate} to ${endDate}`, 14, 40);
      ...
      <main className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {selectedCategory && (
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            {selectedCategory === "Flyers" && flyersData?.length > 0 ? (
            ...
            {selectedCategory === "Coupon Gifts" && couponData?.length > 0 ? (
